...
分母の `233` は、ケース総数 `239` から `unsupported`(3件) と `intentionalDiff`(3件) を
...
本来ここはrune基準と食い違う箇所でした。**本家TS版で修正済み**です。
`testdata/compat/SOURCE` に記録したコミット `3c7cff5e` はこの修正より前なので、
**fixtureを再同期しないと修正後の期待値になりません**（→ 14節）。

### 一致しない3点

Goネイティブにすると差が出るのは、**次の3つ**です。
...
| 5 | `event` と非同期命令 | `10` グループが通過。**全グループ通過（非対応3件・意図的差異3件を除く）** |
