...
#   合計: 180/233 通過 (77.3%)  不一致 53件 / 未実行 0件 / 非対応 3件 / 意図的差異 3件
```
...
| `文字列分解` / `置換` / `出現回数` | コードポイント単位 | 同じ | ○ 一致 |
| `ASC` / `CHR` | コードポイント値 | 同じ | ○ 一致 |
| ZWJ絵文字 `"👨‍👩‍👦"` の `文字数` | 5 | 5 | ○ 一致 |

### 一致しない3点

Goネイティブにすると差が出るのは、**次の3つだけ**です。
いずれも差分fixtureで `intentionalDiff` を付けて集計から外してあります。

| 箇所 | TS版 | Go版 | 備考 |
|---|---|---|---|
| 文字列の `要素数` / `LEN` | `"𩸽あ"` → **3** | 2 | TS版内部で `文字数`(2) と食い違っている |
...
1. **`要素数` / `LEN`**: TS版側の不整合に見えます。同じ文字列に対して
   `文字数` が2、`要素数` が3を返すのは、利用者から見て説明がつきません。
   **TS版をコードポイント基準に揃えるIssueを別に立てる**のが筋で、
   そうなればGo版との差もなくなります（互換性への影響は要調査）
2. **添字アクセス**: サロゲートの片割れは単独で文字として成立しないので、
   Go版がrune基準にするほうが自然です。**意図的差異として受け入れます**
3. **正規表現**: Goの `regexp` はrune基準なので、こちらも意図的差異とします。
...
- **ソース位置を必ず持つ。** `Inst.Pos` から `SourcePos`、さらに `SourceFile` を
  参照してファイル名・行・列を復元する。エラー文面の行番号が互換対象なので落とせない
...
`internal/stdlib` を直接 `import` できません。段階9で最小限の公開ファサードを
...
| 1 | `prepare` / `lexer` / `parser`。ASTまで | 全ケースが構文エラーなく解析できる |
...
| 4 | `event` と非同期命令 | `10` グループが通過。**全グループ通過（非対応3件・意図的差異3件を除く）** |
| 5 | `nodelib`。CUI版の完成 | 実用スクリプトが動く。単一バイナリを配布できる |
| 6 | `bundle`。プログラムとリソースの単一ファイル梱包 | `gonako build` の成果物が別マシンで動く |
| 7 | GUI版（Wails） | IMEで日本語入力ができる |
| 8 | オフィス処理・PDF作成・画像生成 | 各機能のテストが通る |
| 9 | `gogen`。Goコード生成バックエンド | 3系統の差分fixtureが一致する |

**段階5で単体の価値が出ます**（インストール不要のCUI版）。
**段階6で当初の目的が達成されます**（作ったものをそのまま渡せる）。
段階7以降はそれぞれ独立して判断できます。
...
- **`要素数` / `LEN` の文字列に対する挙動**: TS版をコードポイント基準に揃えるか（→ 5節）
