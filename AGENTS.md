...
と1対1対応）。したがって**なでしこの意味論を再実装しているわけではなく**、
`internal/vm` と `internal/gogen` が挙動で食い違うリスクは（新しい命令を
追加したとき以外は）構造的にありません。

値の置き場所だけは、インタプリタと同じにしていません。

| | インタプリタ | 生成コード |
|---|---|---|
| オペランドスタック | `[]value.Value` | 深さごとのGoのローカル変数 `s0` `s1` … |
| 数値と分かったスタック段 | 同上 | 生の `float64` の変数 `f0` `f1` … |
| ローカル変数 | `*value.Cell` の配列 | 同左。ただし数値と分かり、捕捉されないものは `l0` `l1` … |
| 大域変数 | `*value.Cell` の配列 | 同左。ただし数値と分かり、**主処理の中だけで完結する**ものは `g0` `g1` … |

大域変数の条件が厳しいのは、そのセルが生成コードの外からも見えるからです。
ほかの関数が読むなら移せませんし、`表示ログ` のようにシステム側が名前で
読み書きする変数も移せません（`internal/gogen/types.go` の
`promotableGlobals`）。なでしこのトップレベルに書いた変数はすべて大域変数
なので、ここを移せるかどうかが素朴なループの速さをほぼ決めます。
...
| オペランドスタックをGoのローカル変数へ展開する（`push`/`pop` の消去） | gogen |
| 数値型の推論と非ボックス化（`internal/gogen/types.go`。生の `float64` で計算する） | gogen |
| 大域変数の昇格（同上の `promotableGlobals`。主処理の中だけで完結する数値をGoの変数へ） | gogen |
| 添字1つの `A[I]` を `IndexGet1`/`IndexSet1` へ（1要素スライスの割り当てを消す） | gogen |
| なでしこ関数の引数バッファを関数につき1つに使い回す（`borrowN` と同じ約束） | gogen |
| ネイティブ関数のフレームからオペランドスタックを外す（`internal/vm/vm.go` の `callClosure`） | gogen |
| フレームのシステム値をポインタ渡しにして写しを1回に減らす（同上） | VM・gogen |

#### 正しさの基準はテストであって、本家の実装ではない

**構文解析までは本家に合わせ、そこから先の実装は変えてよい**、という
方針です。ASTが合っていて、テストが通るなら、実行時の作りが本家と違って
いても構いません。守る対象は次の3つで、これが「正しさ」の定義です。

| テスト | 何を固定するか |
|---|---|
| 差分fixture（`compat run` → `compat:check`） | 236ケースの出力・変数の値・エラー文面 |
| DocTest（`gonako doctest`） | マニュアルのサンプル1000本あまりの実行結果 |
| `GOGEN_COMPAT=1 go test ./internal/gogen/` | VM実行と生成コードが同じ結果を出すこと |

以前ここには「速度のためにJavaScriptの意味論を曲げないこと」「この変数は
整数だと決めつける最適化はしない」と書いてありました。この制約は外します。
型推論や、型を特殊化した実行経路を入れてよいことにします。

ただし**外し方には条件があります**。テストは網羅ではないので、
「たぶん数値だろう」と当てにいく最適化を入れると、fixtureに無いケースで
静かに壊れます。入れてよいのは**推論で型が証明できたときだけ特殊化する**
（できなければ一般経路へ落とす）という形の最適化です。実際、
`internal/gogen/types.go` の推論はこの形になっています。

数値そのものは引き続き float64 一本です（→ 4節）。「整数だから int64 で
持つ」といった表現の変更はしていません。生の `float64` として持ち回るのは、
**`value.Value` に包むのを省く**という意味であって、精度や NaN・±0 の
扱いを変えるという意味ではありません。

演算子の意味が `internal/ops` の1箇所にある、という原則も変えていません。
型が分かっているときに通る経路は `ops.binaryNumbers` と同じ式で、
一般経路と1ビットも違わないことを `internal/ops/ops_test.go` が
総当たりで確かめています。
