...
func RegisterFunction(sys *core.Core) {
	...
	sys.AddFunc("表示", value.DefArgs{{"の", "を", "と"}}, Println) // 文字列Sを表示 | ひょうじ
	sys.AddVar("表示ログ", "")                                     // 表示した内容 | ひょうじろぐ
	sys.AddFunc("尋", value.DefArgs{{"と", "を"}}, ask)           // 標準入力から入力を得る | たずねる
	// ファイル読み書き
	sys.AddFunc("開", value.DefArgs{{"を", "から"}}, OpenFile)         // ファイルFの内容を全部読む | ひらく
	sys.AddFunc("読", value.DefArgs{{"を", "から"}}, OpenFile)         // ファイルFの内容を全部読む | よむ
	sys.AddFunc("バイナリ読", value.DefArgs{{"を", "から"}}, OpenBinFile)  // ファイルFの内容をバイナリで全部読む | ばいなりよむ
	sys.AddFunc("保存", value.DefArgs{{"を"}, {"に", "へ"}}, WriteFile) // SをファイルFに保存 | ほぞん
	// プロセス
	sys.AddFunc("OS取得", value.DefArgs{}, getOS)  // OSの種類を返す | OSしゅとく
	sys.AddFunc("秒待", value.DefArgs{{""}}, wait) // N秒待つ | びょうまつ
	// パス操作
	sys.AddConst("母艦パス", getBokanPath())                            // スクリプトのパスを保持 | ぼかんぱす
	sys.AddFunc("ファイル名抽出", value.DefArgs{{"から", "の"}}, basename)    // N秒待つ | びょうまつ
	sys.AddFunc("パス抽出", value.DefArgs{{"から", "の"}}, dirname)        // Fのパスを返す | ぱすちゅうしゅつ
	sys.AddFunc("相対パス展開", value.DefArgs{{"を"}, {"で"}}, relatepath)  // 相対パス展開 | そうたいぱすてんかい
	sys.AddFunc("絶対パス変換", value.DefArgs{{"から", "を", "の"}}, abspath) // 絶対パス変換 | ぜったいぱすへんかん
}
