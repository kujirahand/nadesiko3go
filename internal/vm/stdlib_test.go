...
func TestStringCommands(t *testing.T) {
	tests := []struct{ name, code, want string }{
		...
		// 検索語が空文字列のときは0回とする(TS版 #2482 に合わせる)。空文字列同士も含む。
		{"出現回数-検索語が空", `("abc"で""の出現回数)を表示`, "0"},
		{"出現回数-対象も検索語も空", `(""で""の出現回数)を表示`, "0"},
		{"出現回数-対象が空", `(""で"X"の出現回数)を表示`, "0"},
...
func TestArrayCommands(t *testing.T) {
	tests := []struct{ name, code, want string }{
		...
		// #2472 数値文字列・小数の範囲
		{"配列連番作成-数値文字列", `A=「1」から「3」まで配列連番作成` + "\nAをJSONエンコードして表示", "[1,2,3]"},
		{"配列連番作成-小数の範囲", "A=1.5から3.5まで配列連番作成\nAをJSONエンコードして表示", "[1.5,2.5,3.5]"},
		{"配列連番作成-逆順は空", "A=5から1まで配列連番作成\nAをJSONエンコードして表示", "[]"},
		{"配列連番作成-安全整数の最大値", "A=9007199254740991\nB=AからAまで配列連番作成\nBをJSONエンコードして表示", "[9007199254740991]"},
		{"配列要素作成", `A="x"を3だけ配列要素作成` + "\nAを表示", "x,x,x"},
		{"配列要素作成-多次元", `A=0を[2,2]だけ配列要素作成` + "\nAをJSONエンコードして表示", "[[0,0],[0,0]]"},
		{"配列マップ", "F=関数(V)\n(V*2)で戻る\nここまで\nB=[1,2,3]\nA=FをBに配列マップ\nAを表示", "2,4,6"},
		{"配列フィルタ", "F=関数(V)\n(V%2=0)で戻る\nここまで\nB=[1,2,3,4]\nA=FでBを配列フィルタ\nAを表示", "2,4"},
		// 配列への文字列キー代入（プロパティ）は要素数やJSON変換に影響しない
		{"配列プロパティ", "A=[]\nA＠「あ」＝「い」\nA＠「あ」を表示\nAの要素数を表示\nAをJSONエンコードして表示", "い\n0\n[]"},
		{"配列プロパティ-複製", "A=[]\nA＠「あ」＝「い」\nB=Aを配列複製\nB＠「あ」を表示", "い"},
		{"配列プロパティ-参照命令", "A=[]\nA＠「色」＝「赤」\n(Aから「色」を参照)を表示", "赤"},
		{"配列プロパティ-表列取得", "R=[]\nR＠「名前」＝「花子」\nT=[R]\n(Tの「名前」を表列取得)を表示", "花子"},
...
// TestArrayRangeStopsOnExtremeRange は本家 #2472 の回帰テストである。
// 大きな端点・小数ではないが安全整数の外側の値・広すぎる範囲を渡しても
// 止まらず、本家と同じ文面の実行時エラーになることを確かめる。
func TestArrayRangeStopsOnExtremeRange(t *testing.T) {
	tests := []struct{ name, code, want string }{
		{"空文字列", `「」から「」まで配列連番作成`, "『配列連番作成』には有限の数値を指定してください。"},
		{"空白だけ", `「   」から3まで配列連番作成`, "『配列連番作成』には有限の数値を指定してください。"},
		{"数値でない文字列", `「あ」から3まで配列連番作成`, "『配列連番作成』には有限の数値を指定してください。"},
		{"無限大", `A=1÷0。(1からAまで配列連番作成)を表示`, "『配列連番作成』には有限の数値を指定してください。"},
		// 安全整数の範囲外は1を足しても値が変わらずループが進まない
		{"2の53乗", "A=9007199254740992\n(AからAまで配列連番作成)を表示", "『配列連番作成』には絶対値が2の53乗-1(9007199254740991)以下の数値を指定してください。"},
		{"負の2の53乗", "A=0-9007199254740992\n(AからAまで配列連番作成)を表示", "『配列連番作成』には絶対値が2の53乗-1(9007199254740991)以下の数値を指定してください。"},
		{"巨大な範囲", "A=1から100000000000000000000まで配列連番作成\nAを表示", "『配列連番作成』には絶対値が2の53乗-1(9007199254740991)以下の数値を指定してください。"},
		{"要素数上限", "(0から1000000まで配列連番作成)を表示", "『配列連番作成』で生成される配列の要素数が多すぎます。"},
		{"2の52乗付近", "(4503599627370495.5から4503599628370495まで配列連番作成)を表示", "『配列連番作成』で生成される配列の要素数が多すぎます。"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := vm.RunSource(tt.code, "main.nako3", nil)
