...
//
// # これまでに取った値 (Apple M4 / darwin-arm64。測定条件は下記)
//
...
//	                  -18%            -5%              -11%
//	Value 32B化前      20.49ms/3171    7.58ms/ 90932     31.81ms/403343
//	Value 32B化後       5.37ms/3171    4.40ms/ 90933     16.78ms/403343
//	                  -73.8%          -41.9%            -47.3%
//	フレーム再利用後     5.05ms/3172    1.68ms/  3446      7.18ms/  3335
//	                   ±0             -61.8% / -96.2%   -57.2% / -99.2%
//	システム値ポインタ渡し 4.94ms/3160   1.71ms/  3435      6.89ms/  3324
//	                   ±0             -9.6%             -10.2%
//
// 「呼び出し改善」= スタックの事前確保・セルのまとめ確保・定数判定マップの
// 廃止・引数の借用 (internal/vm/vm.go の callClosure)。
//
// 「システム値ポインタ渡し」= callClosure が呼び先のフレームへ渡す
// [SpecialCount]value.Value を、値ではなくポインタで受け渡すようにして、
// 1呼び出しあたりの配列の写しを3回から1回に減らした (issue #24)。同じ
// コミットで、gogenのネイティブ関数のフレームはオペランドスタックを
// 確保しなくなったが、それはVMのベンチには出ない。変更前後のテスト
// バイナリを交互に5回ずつ測った ns/op の中央値。
//
// 「フレーム再利用」= 非エスケープ（Leaf: NumCaptures == 0 かつ OpMakeFunc を
// 含まない）関数のフレーム・localsスライス・Cell配列・スタックスライスの
// プール再利用。これにより再帰・反復呼び出し中の割り当てが事実上ゼロになり、
// Recursion が -62%、Calls が -57% に改善した。
//
// 「命令上限バッチ化」= 基本ブロックの命令数を起動時に求め、実行中の
// m.executed 加算と上限比較を1命令ごとではなくブロック入口で行う。変更前後の
// テストバイナリを交互に各100回・計6標本測り、Loop の中央値は
// 5.163ms→5.034ms。追加の2 allocation は実行前にブロック表を作る分で、
// ホットループ内のアロケーションは引き続きゼロ。
//
...
// Value 32B化も変更前後のテストバイナリを交互に8ラウンド、各20回実行し、
// ns/opの中央値を比較した。B/opは Recursion が15.77MB→10.86MB、Calls が
// 58.91MB→41.31MBに減り、allocs/opは変わっていない。
//
// 残りのディスパッチ費用について。プロファイルでは命令ディスパッチが約40%、
// オペランドスタックの push/pop が約23%を占める。ir.Inst の A/B/C/Pos は
// int32 にして40バイトから20バイトへ縮小済みで、命令列のキャッシュ効率を
// 改善している。命令カウンタを外す案も測ったが2%程度だったので、暴走検知を
// 優先して残した。
//
