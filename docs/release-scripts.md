...
```text
① バージョン更新 & テスト  (just version-update 3.8.6 && just test)
           │
...
| ファイル名 | 対象環境 | 形式 |
|---|---|---|
| `gonako-3.8.6-darwin-arm64.zip` | macOS (Apple Silicon) | CLIバイナリ |
| `gonako-3.8.6-darwin-amd64.zip` | macOS (Intel) | CLIバイナリ |
| `gonako-3.8.6-linux-amd64.zip` | Linux (x86_64) | CLIバイナリ |
| `gonako-3.8.6-linux-arm64.zip` | Linux (aarch64) | CLIバイナリ |
| `gonako-3.8.6-windows-amd64.zip` | Windows (x86_64) | CLIバイナリ（`gonako.exe`） |
| `gonako-gui-3.8.6-darwin-arm64.app.zip` | macOS (Apple Silicon) | GUI App Bundle |
| `gonako-gui-3.8.6-darwin-amd64.app.zip` | macOS (Intel) | GUI App Bundle |
| `gonako-gui-3.8.6-windows-amd64.zip` | Windows (x86_64) | GUI exe（`gonako-gui.exe`） |
| `gonako-gui-3.8.6-linux-amd64.zip` | Linux (x86_64) | GUI実行ファイル |
...
git add -A
git commit -m "chore: release v3.8.6"
git push -u origin release-v3.8.6

# PRを作成してマージ
gh pr create --title "chore: release v3.8.6" --fill
...
引数を省略した場合は `internal/version/version.go` の現在値が使われます（`just publish 3.8.6` のように明示指定も可能）。
