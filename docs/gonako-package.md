...
なでしこで書いたライブラリは `gonako-package/<package-name>/index.nako3` に配置します。

```nako3
!「doctest/index.nako3」を取り込む
```
...
- 接頭辞のない `xxx.nako3`、`package-name/index.nako3` は、次の順に探索します。
...
拡張子は `.nako3` と `.nako` に対応します。パッケージ名だけを指定する `!「doctest」を取り込む` は `doctest/index.nako3` に展開します。
...
`gonako-package/doctest/index.nako3` が抽出・選択・照合・集計を実装します。
