...
- **ツールバー（画面上部）**:
  ...
  - **「☰」メニュー**: AI開発用プロジェクトのひな形作成、実行ファイルへの変換、Go言語ビルド、ショートカットキー一覧などを表示。
- **画面左側（ツールパネル）**:
...
|---|---|
| 表示 | `表示`、`HTML表示` |
| 作成 | `DOM部品作成`、`ラベル作成`、`エディタ作成`、`ボタン作成`、`フォーム作成`、`送信ボタン作成` |
| 作成先 | `DOM親要素設定`（公式別名の`DOM親部品設定`も利用可能）、システム変数`DOM親要素` |
| 値 | `テキスト設定`、`テキスト取得` |
...
`「section」のDOM部品作成`のようにタグ名を指定すると、任意のHTML要素を作成できます。
作成先は通常画面のルートですが、数値ハンドルまたはCSSセレクターを
`DOM親要素設定`へ渡すと、それ以降のDOM部品と既存の作成命令がその要素の子になります。

```nako3
「<main id="app"></main>」をHTML表示
「#app」にDOM親要素設定
見出し=「h2」のDOM部品作成
見出しに「動的な見出し」をDOMテキスト設定
```

作成先の現在値はシステム変数`DOM親要素`が保持します（初期値は画面のルートを表す`0`）。
`DOM親要素設定`はこの変数を更新し、逆に`DOM親要素`へ直接代入しても作成先は切り替わります。
代入する値は`DOM親要素設定`と同じ規則で解決するので、数値ハンドル、CSSセレクター、
id、`0`のいずれでも指定できます。

```nako3
「<div id="left"></div><div id="right"></div>」をHTML表示
「#left」にDOM親要素設定
「左側」のラベル作成
「#right」をDOM要素取得をDOM親要素に代入
「右側」のラベル作成
0にDOM親要素設定        // 画面のルートへ戻す
```

存在しないセレクター、id、ハンドルを指定した場合は実行時エラーになります。
なお`表示`と`HTML表示`は`DOM親要素`の影響を受けず、常に画面のルートへ追加します
（本家wnako3の`表示`が出力領域固定なのと同じ挙動です）。
