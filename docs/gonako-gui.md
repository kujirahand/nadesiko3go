...
| `-theme <文字列>` | 文字列 | なし | テーマ（`ライト`・`ダーク`・`自動`、または`light`・`dark`・`auto`）。指定すると`index.json`やエディタの設定より優先する |
...
2. **コマンドライン引数とフラグの解析**:
   - `flag.FlagSet` で `-dir`, `-url`, `-title`, `-width`, `-height`, `-theme`, `-debug` などを解析（環境変数 `GONAKO_DEBUG` も反映）。
...
### 内蔵エディタの画面構成
- **ツールバー（画面上部）**:
  - **「📄 新規」ボタン**: クリックすると「新規ファイル」（`Ctrl+N` / `Cmd+N` と同じ、編集中の内容をリセットするだけ）と「新規プロジェクト」（フォルダ名を入力してフォルダを作成し、メインファイル`main.nako3`とAGENTS.md/CLAUDE.mdも自動で用意し、`main.nako3`を開いた状態にします）を選べます。
  ...
  - **「☰」メニュー**: 自動整形（→ `docs/format.md`）、AI開発用プロジェクトのひな形作成、実行ファイルへの変換、Go言語ビルド、ショートカットキー一覧などを表示。
- **画面左側（ツールパネル）**:
...
システム定数`母艦`は、実行中のgonako-guiウィンドウを表します。
`ウィンドウ変更`でサイズ・位置・状態・タイトルなどを変更し、`ウィンドウ取得`で
現在値を辞書として取得できます。

```nako3
{"サイズ":[640,480], "位置":"中央"}で母艦のウィンドウ変更。
情報＝母艦のウィンドウ取得。
```

`テーマ`に`"ライト"`・`"ダーク"`・`"自動"`を指定すると、ウィンドウの配色を切り替えられます。
表示中のページの`<html>`には`data-gonako-theme`属性（`light`または`dark`）が付くので、
CSSからテーマに合わせた配色にできます。

```nako3
{"テーマ":"ダーク"}で母艦のウィンドウ変更。
```

HTMLフォルダの`index.json`にも同じ設定を書けます。単一実行ファイルへ変換した場合も
設定ファイルはリソースとして梱包され、ウィンドウを表示する前に適用されます。
設定項目と優先順位の詳細は[`window-size.md`](window-size.md)を参照してください。
...
|---|---|
| 表示 | `表示`、`HTML表示` |
| 作成 | `DOM要素作成`、`DOM部品作成`、`ラベル作成`、`エディタ作成`、`テキストエリア作成`、`ボタン作成`、`フォーム作成`、`送信ボタン作成` |
| 追加部品 | `キャンバス作成`、`画像作成`、`改行作成`、`チェックボックス作成`、`セレクトボックス作成`、`色選択ボックス作成`、`日付選択ボックス作成`、`パスワード入力エディタ作成`、`値指定バー作成` |
| 作成先 | `DOM親要素設定`（公式別名の`DOM親部品設定`も利用可能）、システム変数`DOM親要素` |
| 共通設定 | `DOMスキン設定`、システム変数`DOMスキン`、`DOMスキン辞書`、`DOM部品オプション` |
| 値 | `テキスト設定`、`テキスト取得` |
| DOM検索 | `DOM要素取得`、`DOM要素ID取得`、`DOM要素全取得` |
| DOM内容 | `DOMテキスト変更`、`DOMテキスト取得`、`HTML変更`、`HTML取得` |
| 追加・移動・削除 | `DOM部品作成`、`DOM部品削除` |
...
`DOM要素作成`は要素を画面へ接続せず、数値ハンドルだけを作ります。そのハンドルへ
テキスト、HTML、属性、スタイル、イベントを設定してから`DOM部品作成`へ渡すと、
現在の`DOM親要素`へ追加できます。接続済みのハンドルを別の親のもとで再び
`DOM部品作成`すると、同じ要素が新しい親へ移動します。

```nako3
部品=「article」のDOM要素作成
部品に「準備してから追加」をDOMテキスト設定
部品の「class」に「card」をDOM属性設定
「#left」にDOM親要素設定
部品のDOM部品作成
「#right」にDOM親要素設定
部品のDOM部品作成  // #leftから#rightへ移動
```

`DOM部品削除`には数値ハンドルまたはCSSセレクターを指定できます。対象の子要素と
イベント登録もまとめて破棄され、削除済みハンドルを以後の命令へ渡すとエラーになります。
削除対象が現在の`DOM親要素`またはその祖先なら、追加先は画面ルートの`0`へ戻ります。
自分自身や自分の子を親にする循環した移動もエラーになります。
...
部品へ共通の外観を設定するには、2引数（タグ名と部品ハンドル）を受け取る関数を
`DOMスキン辞書`へ登録し、その名前を`DOMスキン設定`で選びます。選択したスキンは
`DOM部品作成`と既存の作成命令のどちらにも適用されます。

```nako3
枠スキン=関数(タグ,部品)
　部品に{"border":"1px solid navy","padding":"0.5rem"}をDOMスタイル一括設定
ここまで
DOMスキン辞書["枠"]=枠スキン
「枠」にDOMスキン設定
「スキン付き」のラベル作成
...
`DOM部品オプション["自動改行"]`をオンにすると、作成した部品の直後へ`br`要素を
自動で追加します。部品と`br`はどちらも現在の`DOM親要素`へ追加されるため、作成先を
切り替えた場合も同じ親の中に並びます。初期値はオフで、従来どおり改行を追加しません。
...
`セレクトボックス作成`には選択肢の配列を渡します。作成後の選択肢は
`セレクトボックスアイテム設定`で全件を差し替えられます。

```nako3
色一覧=["赤","青","緑"]のセレクトボックス作成
["白","黒"]を色一覧にセレクトボックスアイテム設定
色一覧を変更した時には
　色一覧のテキスト取得を表示
ここまで
```

`キャンバス作成`は`[幅,高さ]`、`値指定バー作成`は`[最小,最大,初期値]`を指定します。
値指定バーの初期値を省略した場合は公式wnako3と同じ計算で補います。チェックボックスは
ラベルを含む部品を作りますが、戻り値はイベントや属性設定に使えるinput要素のハンドルです。

---

## 6. HTMLダイアログを使う

ウィンドウモードでは、`言う`、`尋ねる`、`二択`、`ボタン選択`、`リスト選択`をWebView内の
HTMLダイアログとして表示します。macOSのWebKitでネイティブダイアログAPIに依存しない
よう、プログラムを非同期で実行し、回答が返ってから次の命令へ進みます。
...
- `ボタン選択`は候補の配列をボタンとして並べ、選ばれた文字列を返します。右上の`[x]`で閉じると空文字列を返します。
- `リスト選択`は候補の配列をリストとして表示し、`[OK]`で選択中の項目を、`[キャンセル]`または`[x]`では空文字列を返します。
- どちらも配列の要素0が`#`で始まる場合、それをダイアログ上部のラベルとして扱い、候補からは除きます。
...
| `-theme <文字列>` | テーマ（`ライト`・`ダーク`・`自動`） | `index.json`またはエディタの設定、なければ`自動` |
