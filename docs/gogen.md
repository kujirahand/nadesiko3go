...
CLI（`gonako gengo`）のほか、gonako-guiエディタのハンバーガーメニュー「Go言語でビルド」からも同じことができます（→ 8節）。
...
| 最適化 | 内容 | 効く範囲 |
|---|---|---|
| 定数畳み込み | `(1+2)*3` のようにリテラルだけでできた式を、コンパイル時に定数1つへ潰す（`internal/compiler/fold.go`） | VM・gogen 両方 |
| 数値どうしの高速経路 | 両辺が数値なら `ToPrimitive`/`ParseFloat` を通さず直接 float64 演算（`internal/ops`） | VM・gogen 両方 |
| 定数の直接埋め込み | `m.ConstValue(3)` ではなく `rt.Number(9)` をソースに書く。定数プール参照とkind判定が消える | gogen のみ |
| 演算子の直接呼び出し | 演算子・変数代入をMachineのメソッド経由にしない。Goがインライン展開できる形にする | gogen のみ |

定数畳み込みと高速経路は、実行時とまったく同じ実装（`internal/ops`）を使って行います。畳み込んだ結果と実行して得られる結果が食い違わないこと、高速経路が一般経路と1ビットも違わないこと（NaN・±0を含む）は `internal/ops/ops_test.go` が総当たりで確認しています。

### 実測（Apple Silicon）

| ワークロード | 最適化前のVM | 現在のVM | 現在のgogen |
|---|---|---|---|
| ループ＋算術（500万回） | 1.01s | 0.88s | **0.83s** |
| 再帰（フィボナッチ28） | 0.45s | 0.43s | **0.23s** |

関数呼び出しが多いほどgogenが有利です（インタプリタのフレーム構築とディスパッチが丸ごと消えるため）。算術中心のループでは差は小さくなります。

### 行っていない最適化

- **整数の特殊化**: なでしこの数値はJavaScript同様すべて float64 です（AGENTS.md §4）。「この変数は整数だ」と決めつけて int64 で持つような最適化は、互換性を壊すため行いません。上記の高速経路は、あくまで**実行時に両辺が数値だったとき**に遠回りを省くものです
- **式の木への変換**: 生成コードは今もオペランドスタック（`push`/`pop`）を通します。プロファイル上、残る最大のコストはここです。式を直接Goの式へ組み立てれば更に速くなりますが、バイトコードからの機械的な変換という現在の作りを大きく変えることになります
- **共通部分式の削除・ループ不変式の巻き上げ・インライン展開**（なでしこ関数の）: いずれも未実装です
...
## 8. gonako-guiエディタから使う（Go言語でビルド）

CLIのほか、gonako-guiエディタのハンバーガーメニュー「🔧 Go言語でビルド」からも同じ変換・ビルドができます。開いている `.nako3` ファイルを対象に、次を自動で行います。

1. `internal/gogen` でGoソースを生成する（1〜7節と同じ既定プラグイン。ただし `guilib`（`ウィンドウ作成`）は含めない。→ 3節の理由と同じで、gonako本体をcgo/WebView2に依存させないため）
2. **実行ファイルと同じフォルダに `nadesiko3go/` フォルダ（このリポジトリのソース）が無ければ、GitHubの最新masterを自動でダウンロードして展開する**（`https://github.com/kujirahand/nadesiko3go/archive/refs/heads/master.zip`）。2回目以降は展開済みのものをそのまま使う
3. 一時フォルダに生成したGoソースと、ダウンロード（または既存）のソースを指す `replace` 付きの `go.mod` を書き出す
4. `go mod tidy` → `go build` を実行し、指定した出力先にネイティブ実行ファイルを作る

つまり、**gonako-gui本体さえ配布すれば、それを実行している端末にGoツールチェインさえあれば**、このリポジトリを手元に用意しなくても「Go言語でビルド」が使えます（初回だけネットワーク接続が必要）。ダウンロードしたソースは削除・作り直しが自由です（次回また自動でダウンロードし直します）。

`ウィンドウ作成` を使うプログラムをビルドしたい場合は、この機能ではなくエディタの「実行」ボタン（VM実行）を使ってください。
