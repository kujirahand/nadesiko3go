...
```bash
just version-update 3.8.6   # バージョン番号を一括更新
just test                   # テスト
...
明示するときは `just publish 3.8.6` のように渡します。
...
just homebrew-update "3.8.6 -push"
...
| ファイル名 | 対象 | 形式 |
|---|---|---|
| `gonako-3.8.6-darwin-arm64.zip` | macOS (Apple Silicon) | CLIバイナリzip |
| `gonako-3.8.6-darwin-amd64.zip` | macOS (Intel) | CLIバイナリzip |
| `gonako-3.8.6-linux-amd64.zip` | Linux (x86_64) | CLIバイナリzip |
| `gonako-3.8.6-linux-arm64.zip` | Linux (aarch64) | CLIバイナリzip |
| `gonako-3.8.6-windows-amd64.zip` | Windows (x86_64) | CLIバイナリzip（中身は`gonako.exe`） |
| `gonako-gui-3.8.6-darwin-arm64.app.zip` | macOS (Apple Silicon) | GUI App Bundle zip |
| `gonako-gui-3.8.6-darwin-amd64.app.zip` | macOS (Intel) | GUI App Bundle zip |
| `gonako-gui-3.8.6-windows-amd64.zip` | Windows (x86_64) | GUI exe zip |
| `gonako-gui-3.8.6-linux-amd64.zip` | Linux (x86_64) | GUI実行ファイルzip |
| `upload-3.8.6.sh` | - | GitHub Releasesアップロード用スクリプト（macOS/Linux用） |
| `upload-3.8.6.bat` | - | GitHub Releasesアップロード用バッチ（Windows用） |

---

## 3. GitHub Releases へのアップロード

生成した成果物を GitHub Releases の該当タグ（例: `3.8.6`）にアップロードします。
